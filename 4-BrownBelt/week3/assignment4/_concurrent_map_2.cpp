...
public:
    ...
    {
        WriteAccess(const K &k, Bucket &bucket) : lock_guard(bucket.m), ref_to_value(bucket.data[k]) {}
        V &ref_to_value;
    };

    struct ReadAccess :lock_guard<mutex>
    {
        ReadAccess(const K &k, const Bucket &bucket) : lock_guard(bucket.m), ref_to_value(bucket.data.at(k)) {}
