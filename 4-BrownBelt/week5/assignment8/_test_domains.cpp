...
void TesGetReversedParts() {
    Domain domain("me.ru");
    auto range = domain.GetReversedParts();
    vector<string_view> reversedParts(begin(range), end(range));
    ASSERT_EQUAL(vector<string_view>({ "ru", "me" }), reversedParts);
}

void TestDomainNotItsSubDomain() {
    Domain domain("me.ru");
    Domain subdomain("me.ru");
    ASSERT(IsSubdomain(subdomain, domain));
}

void TestIsSubDomainParamsReversed() {
    ASSERT(IsSubdomain(Domain("me.ru"), Domain("ru")));
}

void TestDomainCheckerConsumeSubdomain() {
    vector<Domain> domains{ Domain("me.ru"), Domain("be.lu") };
    ASSERT(DomainChecker(domains.begin(), domains.end()).IsSubdomain(Domain("ve.me.ru")));
}

void TestCheckDomains()
{
    ASSERT_EQUAL(CheckDomains(vector<Domain>({ Domain("ma.ru") }), vector<Domain>({ Domain("be.ma.ru") })), vector<bool>({ false }));
}

void TesPrintCheckResults() {
    ostringstream stream;
    PrintCheckResults(vector<bool>({ true, false }), stream);
...
int main() {
    TestRunner tr;
    RUN_TEST(tr, TestEmptyStringAdded);
    RUN_TEST(tr, TesGetReversedParts);
    RUN_TEST(tr, TestCheckDomains);
    RUN_TEST(tr, TesPrintCheckResults);
    RUN_TEST(tr, TestDomainCheckerConsumeSubdomain);
    RUN_TEST(tr, TestDomainNotItsSubDomain);
    RUN_TEST(tr, TestIsSubDomainParamsReversed);
