...
void ValidateBounds(Number number_to_check,
    Number min_value,
    Number max_value) {
...
public:
    ...
    void CombineWith(const BulkLinearUpdater& other) {
        ...
        add_.delta.earned =
            add_.delta.earned * other.tax_.factor + other.add_.delta.earned;
        add_.delta.spent += other.add_.delta.spent;
    }

    MoneyState Collapse(MoneyState origin, IndexSegment segment) const {
        return {
            origin.earned * tax_.factor + add_.delta.earned * segment.length(),
...
private:
    ...
    public:
        ...
        Data ProcessPartial(const NodeHolder&,
            IndexSegment,
...
struct EarnRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        ...
        manager.AddBulkOperation(date_segment,
            BulkMoneyAdder{ MoneyState{daily_income, 0.0} });
    }
...
struct PayTaxRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        manager.AddBulkOperation(MakeDateSegment(date_from, date_to),
            BulkTaxApplier::FromPercentage(percentage));
    }
...
struct SpendRequest : ModifyRequest {
    ...
    void ParseFrom(string_view input) override {
        date_from = Date::FromString(ReadToken(input));
        date_to = Date::FromString(ReadToken(input));
        outgo = ConvertToInt(input);
    }

    void Process(BudgetManager& manager) const override {
        const auto date_segment = MakeDateSegment(date_from, date_to);
        const double daily_outgo = outgo * 1.0 / date_segment.length();
        manager.AddBulkOperation(date_segment, BulkMoneyAdder{ 0.0, daily_outgo });
    }
